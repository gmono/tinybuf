...
extern "C"
{
    ...
    typedef int (*tinybuf_plugin_read_fn)(uint8_t tag, buf_ref *buf, tinybuf_value *out, CONTAIN_HANDLER contain_handler, tinybuf_error *r);
    typedef int (*tinybuf_plugin_write_fn)(uint8_t tag, const tinybuf_value *in, buffer *out, tinybuf_error *r);
    typedef int (*tinybuf_plugin_dump_fn)(uint8_t tag, buf_ref *buf, buffer *out, tinybuf_error *r);
    typedef int (*tinybuf_plugin_show_value_fn)(uint8_t tag, const tinybuf_value *in, buffer *out, tinybuf_error *r);
    typedef struct
    {
        const uint8_t *tags;
        int tag_count;
