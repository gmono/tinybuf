...
static int op_str_string(tinybuf_value *self, const tinybuf_value *args, tinybuf_value *out){ (void)args; buffer *s = tinybuf_value_get_string(self); if(!s){ tinybuf_value_init_string(out, "", 0); return 0; } int len=buffer_get_length(s); tinybuf_value_init_string(out, buffer_get_data(s), len); return len; }
static int op_dump_value(tinybuf_value *self, const tinybuf_value *args, tinybuf_value *out){ (void)args; buffer *b = buffer_alloc(); int w = tinybuf_try_write_box(b, self); buffer *t = buffer_alloc(); tinybuf_dump_buffer_as_text(buffer_get_data(b), buffer_get_length(b), t); tinybuf_value_init_string2(out, t); buffer_free(b); return w; }
static int op_desc_value(tinybuf_value *self, const tinybuf_value *args, tinybuf_value *out){ (void)args; const char *name = "value"; tinybuf_value_init_string(out, name, (int)strlen(name)); return (int)strlen(name); }
static int op_linq_count(tinybuf_value *self, const tinybuf_value *args, tinybuf_value *out){ (void)args; int n = tinybuf_value_get_child_size(self); tinybuf_value_init_int(out, n); return 1; }
static int contain_any_local(uint64_t v);
static int op_linq_identity(tinybuf_value *self, const tinybuf_value *args, tinybuf_value *out){ (void)args; buffer *b = buffer_alloc(); int w = tinybuf_try_write_box(b, self); (void)w; tinybuf_value_clear(out); buf_ref br = (buf_ref){ buffer_get_data(b), (int64_t)buffer_get_length(b), buffer_get_data(b), (int64_t)buffer_get_length(b) }; int r = tinybuf_try_read_box(&br, out, contain_any_local); buffer_free(b); return r>0? r : r; }
static int contain_any_local(uint64_t v){ (void)v; return 1; }
...
int tinybuf_register_builtin_plugins(void){
    ...
    const char *display_methods[2] = { "__str__", "display" }; tinybuf_trait_register("display", display_methods, 2);
    const char *sdisplay_methods[2] = { "__dump__", "serialized_display" }; tinybuf_trait_register("serialized_display", sdisplay_methods, 2);
    const char *linq_methods[4] = { "map", "filter", "reduce", "count" }; tinybuf_trait_register("linq", linq_methods, 4);
    tinybuf_oop_register_type("string"); tinybuf_oop_register_op("string", "__str__", "string->string", "str", op_str_string); tinybuf_oop_register_op("string", "display", "string->string", "display", op_str_string); tinybuf_oop_register_op("string", "__dump__", "string->text", "dump", op_dump_value); tinybuf_oop_register_op("string", "serialized_display", "string->text", "sdisplay", op_dump_value); tinybuf_trait_attach("string", "display"); tinybuf_trait_attach("string", "serialized_display");
    tinybuf_oop_register_type("hetero_list"); tinybuf_oop_register_op("hetero_list", "map", "array->array", "map", op_linq_identity); tinybuf_oop_register_op("hetero_list", "filter", "array->array", "filter", op_linq_identity); tinybuf_oop_register_op("hetero_list", "reduce", "array->int", "reduce", op_linq_count); tinybuf_oop_register_op("hetero_list", "count", "array->int", "count", op_linq_count); tinybuf_oop_register_op("hetero_list", "__dump__", "array->text", "dump", op_dump_value); tinybuf_trait_attach("hetero_list", "linq"); tinybuf_trait_attach("hetero_list", "serialized_display");
    tinybuf_oop_register_type("dataframe"); tinybuf_oop_register_op("dataframe", "__str__", "df->string", "str", op_str_string); tinybuf_oop_register_op("dataframe", "display", "df->string", "display", op_desc_value); tinybuf_oop_register_op("dataframe", "__dump__", "df->text", "dump", op_dump_value); tinybuf_oop_register_op("dataframe", "serialized_display", "df->text", "sdisplay", op_dump_value); tinybuf_trait_attach("dataframe", "display"); tinybuf_trait_attach("dataframe", "serialized_display");
    return r;
}
...
typedef struct { char *name; char **methods; int method_count; int method_cap; } trait_entry;
static trait_entry *s_traits = NULL; static int s_traits_count = 0; static int s_traits_cap = 0;
static int trait_index_by_name(const char *name){ if(!name) return -1; for(int i=0;i<s_traits_count;++i){ if(s_traits[i].name && strcmp(s_traits[i].name, name)==0) return i; } return -1; }
int tinybuf_trait_register(const char *trait_name, const char **methods, int method_count){ if(!trait_name || (!methods && method_count>0)) return -1; int idx = trait_index_by_name(trait_name); if(idx>=0) return 0; if(s_traits_count==s_traits_cap){ int nc=s_traits_cap? s_traits_cap*2:8; s_traits=(trait_entry*)tinybuf_realloc(s_traits,sizeof(trait_entry)*nc); s_traits_cap=nc; } int tl=(int)strlen(trait_name); trait_entry e; e.name=(char*)tinybuf_malloc(tl+1); memcpy(e.name,trait_name,tl); e.name[tl]='\0'; e.method_count=method_count; e.method_cap=method_count; e.methods=NULL; if(method_count>0){ e.methods=(char**)tinybuf_malloc(sizeof(char*)*method_count); for(int i=0;i<method_count;++i){ int ml=(int)strlen(methods[i]); e.methods[i]=(char*)tinybuf_malloc(ml+1); memcpy(e.methods[i],methods[i],ml); e.methods[i][ml]='\0'; } } s_traits[s_traits_count++]=e; return 0; }
static int type_has_method(int type_idx, const char *method){ if(type_idx<0||type_idx>=s_oop_count||!method) return 0; for(int i=0;i<s_oop[type_idx].op_count;++i){ if(s_oop[type_idx].op_names[i] && strcmp(s_oop[type_idx].op_names[i], method)==0) return 1; } return 0; }
int tinybuf_trait_attach(const char *type_name, const char *trait_name){ int ti=oop_index_by_name(type_name); int ri=trait_index_by_name(trait_name); if(ti<0||ri<0) return -1; for(int i=0;i<s_traits[ri].method_count;++i){ if(!type_has_method(ti, s_traits[ri].methods[i])) return -1; } return 0; }
int tinybuf_trait_has(const char *type_name, const char *trait_name){ int ti=oop_index_by_name(type_name); int ri=trait_index_by_name(trait_name); if(ti<0||ri<0) return 0; for(int i=0;i<s_traits[ri].method_count;++i){ if(!type_has_method(ti, s_traits[ri].methods[i])) return 0; } return 1; }
int tinybuf_trait_do(const char *type_name, const char *trait_name, const char *method_name, tinybuf_value *self, const tinybuf_value *args, tinybuf_value *out){ if(!tinybuf_trait_has(type_name, trait_name)) return -1; return tinybuf_oop_do_op(type_name, method_name, self, args, out); }
