...
#define TB_STRUCT_BEGIN(T) typedef struct T T; struct T {
...
// ---- trait macros ----
#define TB_TRAIT_BEGIN(TR) typedef struct TR##_vtable TR##_vtable; struct TR##_vtable {
...
#if defined(_WIN32)
#include <windows.h>
typedef struct { volatile LONG state; } tb_spinlock_t;
...
static inline void tb_spinlock_lock(tb_spinlock_t *lk){ while(InterlockedCompareExchange(&lk->state, 1, 0) != 0) { YieldProcessor(); } }
static inline void tb_spinlock_unlock(tb_spinlock_t *lk){ InterlockedExchange(&lk->state, 0); }
#else
#include <stdatomic.h>
typedef struct { atomic_flag flag; } tb_spinlock_t;
...
static inline void tb_spinlock_lock(tb_spinlock_t *lk){ while(atomic_flag_test_and_set(&lk->flag)) { ; } }
