            ...
            while(true){
                if(g_in_test.load()){ 
                    auto now = std::chrono::steady_clock::now();
                     ...
                     long long st = g_start_ms.load(); int to = g_timeout_ms.load(); 
                     if(st>0 && to>0 && ms - st > to){ 
                        LOGE("TEST TIMEOUT: %s exceeded %d ms", g_current_test_name.c_str(), to);
                         std::_Exit(3); } 
                    }
...
struct TestLogger : Catch::EventListenerBase
{
    ...
    {
        _ensure_watchdog();
        g_current_test_name = info.name;
        g_in_test.store(1);
        auto now = std::chrono::steady_clock::now();
        g_start_ms.store(std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count());
        LOGI("TEST ENTER: %s", info.name.c_str());
    }
    void testCaseEnded(Catch::TestCaseStats const &) override
    {
        g_in_test.store(0);
        g_start_ms.store(0);
        LOGI("TEST EXIT: %s", g_current_test_name.c_str());
    }
};
