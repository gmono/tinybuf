...
TEST_CASE("system.extend plugin loads and handles hetero_tuple", "[plugin]"){
    ...
    char msgs[256]; tinybuf_result_format_msgs(&rr, msgs, sizeof(msgs));
    CAPTURE(rr.res, msgs);
    REQUIRE(rr.res > 0);
...
TEST_CASE("system.extend plugin handles dataframe", "[plugin]"){
    ...
    buffer *payload = buffer_alloc(); int w = tinybuf_try_write_custom_id_box(payload, "dataframe", df);
    REQUIRE(w > 0);
    ...
    tinybuf_result rr = tinybuf_try_read_box_r(&br2, out, NULL);
    char msgs2[256]; tinybuf_result_format_msgs(&rr, msgs2, sizeof(msgs2));
    CAPTURE(rr.res, msgs2);
    REQUIRE(rr.res > 0);
    REQUIRE(tinybuf_result_msg_count(&rr) == 0);
    ...
    tinybuf_value_free(out); buffer_free(payload); tinybuf_value_free(df); tinybuf_value_free(cols); tinybuf_value_free(rows); tinybuf_value_free(ten); tinybuf_set_use_strpool(0);
}
...
TEST_CASE("custom result wrappers", "[result]"){
    ...
    tinybuf_register_builtin_plugins();
    tinybuf_value *out = tinybuf_value_alloc();
    tinybuf_result rr = tinybuf_custom_try_read_r("string", (const uint8_t*)"abc", 3, out, NULL);
    REQUIRE(rr.res == 3);
    REQUIRE(tinybuf_result_msg_count(&rr) == 0);
    REQUIRE(tinybuf_value_get_type(out) == tinybuf_string);
    tinybuf_result rf = tinybuf_custom_try_read_r("unknown", (const uint8_t*)"x", 1, out, NULL);
    char msgs3[256]; tinybuf_result_format_msgs(&rf, msgs3, sizeof(msgs3));
